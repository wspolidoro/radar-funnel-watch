...
export default function ClientAlerts() {
  ...
  // Fetch user's alerts only (RLS handles filtering)
  const { data: alerts, isLoading } = useQuery({
    queryKey: ['client-data-leak-alerts', severityFilter, statusFilter],
    queryFn: async () => {
      let query = supabase
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['client-data-leak-alerts'] });
    }
  });
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          ...
          Meus Alertas de Vazamento
        </h1>
        <p className="text-muted-foreground mt-1">
          Monitore alertas de vazamento de dados dos seus emails
        </p>
      </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Você não possui alertas de vazamento no momento</p>
            </div>
          )}
        </CardContent>
      </Card>
